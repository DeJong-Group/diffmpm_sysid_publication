...
# experiment options
obs_choices = ['full', 'row', '2']
optim_choices = ['grad', 'lbfgs']

obs = obs_choices[0]
optim = optim_choices[0]
...
def compute_loss():
    for i in range(steps - 1):
        if obs == "2":
            dist = (target_strain[i, 40] - strain[i, 40]) ** 2
            loss[None] += 0.5 * (dist[0, 0])
            dist = (target_strain[i, 41] - strain[i, 41]) ** 2
            loss[None] += 0.5 * (dist[0, 0])
        elif obs =="row":
            for j in range(Nx):
...
with open(f"result_{obs}_{optim}.json", "w") as outfile: 
